...
class Encoder(nn.Module):
    def __init__(self, args) -> None:
        ...
        self.c_in = args['seq2seq']['c_in'] # channel number ?
        self.c_enc = args['seq2seq']['c_enc'] # out channel
        self.k_enc = args['seq2seq']['k_enc']  # kernel size
        self.s_enc = args['seq2seq']['s_enc'] # stride
    ...
    def forward(self, x):
        for conv in self.conv:
            #print('encoder conv',x.shape,conv)
            x = conv(x)
        x = self.res(x)
